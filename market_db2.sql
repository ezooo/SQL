...
alter view v_viewtest1 as
	select b.mem_id as '회원 아이디', m.mem_name as '회원 이름', b.prod_name as "제품 이름", concat(m.phone1, m.phone2) as "연락처"
    from buy b 
		inner join member m 
        on b.mem_id = m.mem_id;
...
as
	select * from member where height >= 167
    with check option;
insert into v_height167 values('tob', '텔토비',6,'서울',null, null,159,'2005-01-01');
